...
public class WebSearchParameters {
    ...
    public AnnotationSettings getPreferencesFromSearchParameters(UpdatedSearchParameterFileUtility searchParameters) {
        ...
        annotationSettings.clearIonTypes();
        searchParameters.getForwardIons().stream().map((ion) -> {
            annotationSettings.addIonType(Ion.IonType.PEPTIDE_FRAGMENT_ION, ion);
            return ion;
        }).forEachOrdered((ion) -> {
            annotationSettings.addIonType(Ion.IonType.TAG_FRAGMENT_ION, ion);
        });
        ...
        annotationSettings.addIonType(Ion.IonType.REPORTER_ION);
        annotationSettings.addIonType(Ion.IonType.RELATED_ION);
        annotationSettings.setFragmentIonAccuracy(searchParameters.getFragmentIonAccuracy());
        annotationSettings.setFragmentIonPpm(SearchParameters.MassAccuracyType.valueOf(searchParameters.getFragmentAccuracyType()) == SearchParameters.MassAccuracyType.PPM);
        ...
        if (annotationSettings.getReporterIons()) {
            ...
            annotationSettings.getIonTypes().put(ReporterIon.IonType.REPORTER_ION, ptmReporterIons);
        }
